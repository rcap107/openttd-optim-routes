...
def find_path(start, end, cost_map):
    ...
    g_score = { (y,x): float('inf') for y in range(cost_map.shape[0]) for x in range(cost_map.shape[1]) }
    g_score[start] = 0
    
    # f_score: g_score + heuristic
    f_score = { (y,x): float('inf') for y in range(cost_map.shape[0]) for x in range(cost_map.shape[1]) }
    ...
    with tqdm(total=cost_map.shape[0] * cost_map.shape[1], desc="Finding Path") as pbar:
        while open_set:
            ...
            for neighbor in neighbors:
                ...
                move_cost = abs(int(cost_map[current[0], current[1]]) - int(cost_map[neighbor[0], neighbor[1]]))
